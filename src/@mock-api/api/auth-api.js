...
let usersApi = "http://sr.yazilimocagi.net:9063";

/* eslint-disable camelcase */

mock.onGet('/api/auth/sign-in').reply(async () => {
...
mock.onPost('/api/auth/sign-up').reply((request) => {
  ...
  const { displayName, password, username } = data;
  const isusernameExists = usersApi.find((_user) => _user.data.username === username);
  const error = [];

  if (isusernameExists) {
    error.push({
      type: 'username',
      message: 'The username address is already in use',
    });
  }
  ...
  if (error.length === 0) {
    const newUser = {
      ...
      data: {
        displayName,
        photoURL: 'public/assets/images/avatars/Abbott.jpg',
        username,
        settings: {},
        shortcuts: [],
      },
